...
import { X, BatteryCharging, Activity, Zap, Gauge, TrendingUp, TrendingDown, Settings } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from "recharts";
import ComponentQueryDialog from "./component-query-dialog";
import { useComponentQuery } from "../hooks/use-component-query";

interface BateriaDashboardProps {
  bateriaId: string;
  bateriaName: string;
  onClose: () => void;
}

export default function BateriaDashboard({ bateriaId, bateriaName, onClose }: BateriaDashboardProps) {
  const [showQueryDialog, setShowQueryDialog] = useState(false);
  
  // Hook para gestionar consultas de componente
  const { config, data: queryData, isLoading: queryLoading, executeQuery } = useComponentQuery(bateriaId);
  
  // Datos simulados para la batería
        ...
        <Card className="bg-gradient-to-br from-pink-50 to-pink-100">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-pink-700 flex items-center">
              <TrendingUp className="w-4 h-4 mr-2" /> Estado
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-pink-900">{batteryData.status}</div>
          </CardContent>
        </Card>
      </div>
      <div className="bg-white rounded-lg shadow p-4">
        <h3 className="text-lg font-semibold mb-2">Histórico de Voltaje y SOC</h3>
        <ResponsiveContainer width="100%" height={250}>
          <LineChart data={historicalData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="time" />
            <YAxis yAxisId="left" domain={[44, 52]} />
            <YAxis yAxisId="right" orientation="right" domain={[0, 100]} />
            <Tooltip />
            <Legend />
            <Line yAxisId="left" type="monotone" dataKey="voltage" stroke="#22c55e" name="Voltaje (V)" />
            <Line yAxisId="right" type="monotone" dataKey="soc" stroke="#a21caf" name="SOC (%)" />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
